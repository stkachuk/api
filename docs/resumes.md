...
получении детальной истории просмотров. На данный момент этот метод недоступен в API.
...
Если операция публикации резюме не доступна из-за отсутствия прав — `403 Forbidden` (например, для работодателя).
 ...
 * Минимальный размер желаемой компенсации зависит от выбранных специализаций.

 * Часть полей резюме явлюяются readonly, их нельзя изменить при помощи POST/PUT на /resumes. Эти поля: `_progress, photo, updated_at, moderation_note, blocked, finished`.
